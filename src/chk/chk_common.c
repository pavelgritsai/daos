...
daos_unit_oid_t		chk_dummy_obj = { 0 };
daos_key_t		chk_dummy_key = { 0 };
...
void
chk_ranks_dump_by_bitmap(uint32_t rank_nr, uint32_t max, uint8_t *bitmap)
{
	char		 buf[128];
	char		*ptr = buf;
	uint32_t	 max_rank = max * sizeof(uint8_t);
	int		 rc;
	int		 i;
	int		 j;
	...
	for (i = 0, j = 0; i < max_rank && j < rank_nr; i++) {
		if (isclr(bitmap, i))
			continue;

		if (j % 8 == 0)
			rc = snprintf(ptr, 127, "%10u", j);
		else
			rc = snprintf(ptr, 127, " %10u", j);
...
int
chk_bitmap2ranklist(uint32_t rank_nr, d_rank_t max_rank, uint8_t *bitmap, d_rank_list_t **rlist)
{
	...
	result = d_rank_list_alloc(rank_nr);
	if (result == NULL)
		D_GOTO(out, rc = -DER_NOMEM);
		 ...
		 struct chk_property *prop, d_rank_list_t **rlist)
{
	...
	if (rank_nr == 0) {
		...
		if (list_only) {
			for (i = 0; i < rank_nr; i++)
				result->rl_ranks[i] = ranks[i];
		} else {
			for (i = 0; i < rank_nr; i++) {
				result->rl_ranks[i] = ranks[i];
				if (max_rank < ranks[i])
					max_rank = ranks[i];
				setbit(prop->cp_ranks_bitmap, ranks[i]);
			}

			prop->cp_rank_nr = rank_nr;
			prop->cp_bitmap_sz = (max_rank + sizeof(uint8_t) - 1) / sizeof(uint8_t);
		}
...
chk_pool_add_shard(daos_handle_t hdl, d_list_t *head, uuid_t uuid, d_rank_t rank,
		   uint32_t phase, struct chk_bookmark *bk, struct chk_instance *ins, void *data)
{
