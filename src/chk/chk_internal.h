...
/* XXX: This will be replaced by ds_pool_clue in Liwei's patch. */
struct ds_pool_clue {
	uuid_t		 dpc_uuid;
	d_rank_t	 dpc_rank;
	void		*dpc_data;
...
extern daos_unit_oid_t		chk_dummy_obj;
extern daos_key_t		chk_dummy_key;
...
/*
 * \return	Positive value for the count of chk_query_pool_shard.
...
int chk_leader_report(uint64_t gen, uint32_t cla, uint32_t act, int32_t result, d_rank_t rank,
		      uint32_t target, char *pool, char *cont, daos_unit_oid_t *obj,
