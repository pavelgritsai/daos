...
init:
	...
	rc = chk_prop_prepare(rank_nr, ranks, policy_nr, policies, pool_nr, pools,
			      *flags, phase, dss_self_rank(), false, prop, rlist);
...
chk_leader_start_cb(void *args, uint32_t rank, uint32_t phase, int result, void *data, uint32_t nr)
{
	...
	/* TBD: more work after integrated with Liwei's patch. */
	...
	for (i = 0; i < nr; i++) {
		rc = chk_pool_add_shard(csa->csa_hdl, &csa->csa_list, clues[i].dpc_uuid,
					clues[i].dpc_rank, 0, NULL, csa->csa_ins,
					clues[i].dpc_data);
		if (rc != 0)
			goto out;
	}
...
chk_leader_stop_cb(void *args, uint32_t rank, uint32_t phase, int result, void *data, uint32_t nr)
{
	...
	if (rc != 0)
		D_ERROR("Check leader (gen "DF_X64") failed to handle stop CB with ranks %u: "
			DF_RC"\n", csa->csa_ins->ci_bk.cb_gen, rank, DP_RC(rc));
...
chk_leader_stop(uint32_t pool_nr, uuid_t pools[])
{
	...
	/*
	 * XXX: Current engine maybe not the original instance leader that may be crashed.
	 *	Under such case, if want to stop the former instance, then have to restart
	 *	related engines. It will be improved via some force option in the future.
	 */
...
remote:
	...
	rc = chk_stop_remote(ranks, cbk->cb_gen, pool_nr, pools, chk_leader_stop_cb, csa);
	if (rc != 0)
		goto out;
...
int
chk_leader_query(uint32_t pool_nr, uuid_t pools[], chk_query_cb_t query_cb, void *buf)
{
	struct chk_bookmark	*cbk = &chk_leader->ci_bk;
	struct chk_sched_args	*csa = NULL;
	int			 rc;
			  ...
			  enum crt_event_type type, void *arg)
{
	...
	if (rank >= prop->cp_bitmap_sz * sizeof(uint8_t) || !isclr(prop->cp_ranks_bitmap, rank))
		goto out;
	...
	rc = crt_group_secondary_modify(chk_leader->ci_iv_group, rank_list, rank_list,
	...
	if (d_list_empty(&chk_leader->ci_rank_list))
		...
		rc = chk_mark_remote(rank_list, cbk->cb_gen, rank, version);
...
chk_leader_report(uint64_t gen, uint32_t cla, uint32_t act, int32_t result, d_rank_t rank,
		  uint32_t target, char *pool, char *cont, daos_unit_oid_t *obj, daos_key_t *dkey,
		  daos_key_t *akey, char *msg, uint32_t option_nr, uint32_t *options,
		  uint32_t detail_nr, d_sg_list_t *details, bool local, uint64_t *seq)
{
	struct chk_bookmark	*cbk = &chk_leader->ci_bk;
	struct chk_pending_rec	*cpr = NULL;
	int			 rc;
...
chk_leader_notify(uint64_t gen, d_rank_t rank, uint32_t phase, uint32_t status)
{
	...
	case CHK__CHECK_INST_STATUS__CIS_COMPLETED:
		...
		D_GOTO(log, rc = (rc == -DER_ENOENT ? 0 : rc));
...
log:
	D_CDEBUG(rc != 0, DLOG_ERR, DLOG_INFO,
		 "Check leader (gen "DF_X64") handle notification from rank %u, "
		 "phase %u, status %u: "DF_RC"\n", cbk->cb_gen, rank, phase, status, DP_RC(rc));
...
chk_leader_rejoin(uint64_t gen, d_rank_t rank, uint32_t phase)
{
	...
	if (isclr(chk_leader->ci_prop.cp_ranks_bitmap, rank))
...
chk_leader_init(void)
{
	...
	rc = chk_prop_fetch(&chk_leader->ci_prop);
	if (rc == -DER_ENOENT)
		rc = 0;
