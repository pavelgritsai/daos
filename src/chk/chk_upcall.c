		  ...
		  uint32_t *options, uint32_t detail_nr, d_sg_list_t *details)
{
	...
	time_t			  tm = time(NULL);
	char			  objid_buf[OBJID_STR_SIZE];
	char			  dkey_buf[DF_KEY_STR_SIZE];
	char			  akey_buf[DF_KEY_STR_SIZE];
	...
	report.pool_uuid = pool;
	report.cont_uuid = cont;

	if (obj != NULL && !daos_unit_oid_is_null(*obj)) {
		snprintf(objid_buf, OBJID_STR_SIZE - 1, DF_UOID, DP_UOID(*obj));
		report.objid = objid_buf;
	} else {
		report.objid = NULL;
	}

	if (dkey != NULL && !daos_key_is_null(*dkey)) {
		snprintf(dkey_buf, DF_KEY_STR_SIZE - 1, DF_KEY, DP_KEY(dkey));
		report.dkey = dkey_buf;
	} else {
		report.dkey = NULL;
	}

	if (akey != NULL && !daos_key_is_null(*akey)) {
		snprintf(akey_buf, DF_KEY_STR_SIZE - 1, DF_KEY, DP_KEY(akey));
		report.akey = akey_buf;
	} else {
		report.akey = NULL;
	}

	snprintf(time_buf, TIME_STR_SIZE - 1, "%s", ctime(&tm));
