...
static int
dump_value_option_parse(struct ddb_ctx *ctx, struct dump_value_options *cmd_args, struct argv_parsed *argc_v)
{
	char		 *options_short = "";
	int		  index = 0, opt;
	uint32_t	  argc = argc_v->ap_argc;
...
static int
dump_ilog_option_parse(struct ddb_ctx *ctx, struct dump_ilog_options *cmd_args, struct argv_parsed *argc_v)
{
	char		 *options_short = "";
	int		  index = 0, opt;
	uint32_t	  argc = argc_v->ap_argc;
...
static int
process_ilog_option_parse(struct ddb_ctx *ctx, struct process_ilog_options *cmd_args, struct argv_parsed *argc_v)
{
	char		 *options_short = "";
	int		  index = 0, opt;
	uint32_t	  argc = argc_v->ap_argc;
...
static int
rm_ilog_option_parse(struct ddb_ctx *ctx, struct rm_ilog_options *cmd_args, struct argv_parsed *argc_v)
{
	char		 *options_short = "";
	int		  index = 0, opt;
	uint32_t	  argc = argc_v->ap_argc;
...
static int
dump_dtx_option_parse(struct ddb_ctx *ctx, struct dump_dtx_options *cmd_args, struct argv_parsed *argc_v)
{
...
static int
clear_dtx_option_parse(struct ddb_ctx *ctx, struct clear_dtx_options *cmd_args, struct argv_parsed *argc_v)
{
	char		 *options_short = "";
	int		  index = 0, opt;
	uint32_t	  argc = argc_v->ap_argc;
...
ddb_parse_cmd_args(struct ddb_ctx *ctx, struct argv_parsed *parsed, struct ddb_cmd_info *info)
{
	...
	if (same(cmd, COMMAND_NAME_PROCESS_ILOG)) {
		...
		return process_ilog_option_parse(ctx, &info->dci_cmd_option.dci_process_ilog, parsed);
	}
...
ddb_run_help(struct ddb_ctx *ctx)
{
	...
	ddb_print(ctx, "   quit\t\t\n");
	ddb_print(ctx, "   ls\t\tList the containers, objects, dkeys, akeys, or value depending\n");
