...
obj_bulk_args_fini(struct obj_bulk_args *args)
{
	...
	return rc ? dss_abterr2der(rc) : *status;
}
...
rpc2orig_ctx(crt_rpc_t *rpc, bool *is_primary)
{
	/*
	 * TODO:
	 * - Get original cart context by Cart APIs crt_req_dst_provider_get()?
	 * - If multiple secondary targets are specified, is the Cart API sufficient
	 *   to get the original context?
	 * - How to deal with forwarded RPC (it's created on server using primary context)?
	 */
	*is_primary = true;
	return rpc->cr_ctx;
}
