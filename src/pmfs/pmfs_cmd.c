...
pmfs_mount_start(daos_handle_t poh, daos_handle_t coh, struct pmfs **pmfs)
{
	...
	if (rc != 0) {
		printf("pmfs mount cmd process thread failed: %d\n", rc);
		return rc;
	}
		  ...
		  const char *new_name)
{
	...
	static pthread_t rename_thread;
	int rc = 0;
	...
	rc = pthread_create(&rename_thread, NULL, (void *)pmfs_rename_cb, &rename_args);
	if (rc != 0) {
		return rc;
	}

	rc = pthread_join(rename_thread, NULL);
	if (rc != 0) {
		return rc;
	}
...
pmfs_truncate_start(struct pmfs *pmfs, struct pmfs_obj *obj, daos_size_t len)
{
	...
	static pthread_t truncate_thread;
	int rc = 0;
	...
	rc = pthread_create(&truncate_thread, NULL, (void *)pmfs_truncate_cb, &truncate_args);
	if (rc != 0) {
		return rc;
	}

	rc = pthread_join(truncate_thread, NULL);
	if (rc != 0) {
		return rc;
	}
...
pmfs_start_mount(struct pmfs_pool *pmfs_pool, struct pmfs *pmfs)
{
	...
	printf("---------------start scan pool---------------------------\r\n");
	printf("---------------rebuild container list before mount-------\r\n");
	rc = pmfs_init_pool(g_vfca, ctx);
	if (rc != 0) {
		return NULL;
	}

	printf("---------------rebuild container list done---------------\r\n");
	/* start mount thread */
