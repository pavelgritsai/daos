...
app_send_thread_test_pmfs_cmds_in_pool(struct pmfs *pmfs)
{
	...
	rc = pmfs_write_start(pmfs, obj, &user_sgl, 0, &write_size);
	if (rc != 0) {
	...
	daos_size_t read_size = 4096;

	rc = pmfs_read_start(pmfs, obj, &user_sgl, 0, &read_size);
	if (rc != 0) {
...
main(int argc, char **argv)
{
	...
	pmfs_pool = pmfs_init_target_env();
